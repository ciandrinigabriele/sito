...
for (const page of pages) {
  ...
  const shouldBeNoindex = !isProduction || ['/workbook-stato-attuale/', '/workbook-paure-limiti-vincoli/', '/bussola-valori-obiettivo/', '/grazie-per-il-workbook/', '/proposta-percorso/'].includes(page.route)
...
for (const [route, count] of inbound) {
  if (!['/', '/bussola-valori-obiettivo/', '/grazie-per-il-workbook/', '/proposta-percorso/'].includes(route) && count === 0) issues.push({ severity: 'warning', route, issue: 'pagina orfana nel corpo HTML statico' })
}
