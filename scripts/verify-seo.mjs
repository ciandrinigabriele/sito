...
const expectedRoutes = new Set(['/', '/articoli/', '/libro-respira-immagina-agisci/', '/about-2/', '/privacy-policy/', '/cookie-policy/', '/cambia-direzione/', '/inizia/', ...inventory.routes.map((item) => item.route)])
const failures = []
...
const privateRoutes = ['/workbook-stato-attuale/', '/grazie-per-il-workbook/', '/proposta-percorso/']
